...
- 풀네임: Parsing
- 한국어 발음: 파싱
- 뜻: 파일명이나 JSON 같은 원본 문자열/데이터에서 필요한 정보만 뽑아내는 작업
- 프로젝트에서의 의미:
  - UTKFace 파일명에서 age, gender, race를 자동 추출
  - AIHub JSON 라벨에서 나이, 성별, 이미지 매핑 정보를 추출
  - 원본 데이터를 모델 학습용 CSV metadata로 바꾸기 위해 필요했던 전처리

예를 들어 `32_0_1.jpg` 같은 파일명에서 `나이=32`, `성별=0`, `인종=1`을 읽어내는 작업이 파싱입니다.
...
### 8.1 Age Estimation Validation MAE
...
### 8.2 Age Estimation Validation Loss
...
### 8.3 Best Validation MAE by Age Model
...
### 8.4 ConvNeXt-Tiny Test MAE Improvement
...
### 8.5 Gender Classification Metrics
...
### 9.6 Training / Tuning / Fine-tuning
