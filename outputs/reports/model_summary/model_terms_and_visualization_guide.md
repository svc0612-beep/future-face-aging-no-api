...
| 모델 | 풀네임 / 발음 | 장점 | 단점 | 현재 판단 |
|---|---|---|---|---|
| SAM | Style-based Age Manipulation / 샘 | 눈, 코, 입, 눈썹, 얼굴 윤곽 같은 인물 정체성 요소를 비교적 잘 유지하면서 target age를 직접 지정할 수 있습니다. API 없이 로컬 실행이 가능하고, LATS보다 얼굴 붕괴가 적으며 주름·피부 질감 변화가 잘 보입니다. | 가중치 파일과 모델 구조가 커서 용량이 크고 실행 시간이 오래 걸립니다. GPU 메모리 부담도 크며, FFHQ 중심 공개 가중치라 한국인 얼굴 느낌과 성별 특징이 완벽하게 유지되지 않을 수 있습니다. | SAM의 장점이 다른 후보보다 좋아 보여 단점은 감수하고 모델을 선택했습니다. 단, 결과 문구는 반드시 재미용 AI 시뮬레이션으로 안내합니다. |
| LATS | Lifespan Age Transformation Synthesis / 래츠 | 여러 나이대 얼굴을 한 번에 다루는 lifespan aging 방향의 모델이라 프로젝트 목표와 비슷합니다. 젊은 얼굴부터 노년 얼굴까지 변화 흐름을 만들 수 있다는 연구 방향은 장점입니다. | 테스트 결과 눈·코·입·얼굴 윤곽 유지가 약했고, 한국인 얼굴이 서양인처럼 바뀌는 문제가 보였습니다. 여성 얼굴에 원치 않는 수염이나 성별 변화가 생기고, 피부가 자연스럽게 늙기보다 무너지는 느낌이 있었습니다. | 최종 생성기에서 제외. 실패 후보로 기록합니다. |
| CUSP | Custom Structure Preservation in Face Aging / 커스프 | 이름 그대로 얼굴 구조 보존을 강조하므로 눈, 코, 입, 얼굴형을 유지하면서 나이만 변화시키려는 목표와 잘 맞습니다. 한국인 정체성 유지 문제를 해결할 후보로 검토할 가치가 있습니다. | 공식 가중치 확보가 어렵고 Docker/연구 코드 중심이라 설치와 실행 난도가 높습니다. 현재 프로젝트 일정과 MVP 구조에서는 바로 연결해 테스트하기 어렵습니다. | 보류. 추후 가중치와 실행 환경이 확보되면 재검토합니다. |
| Rule-based Aging Filter | 규칙 기반 에이징 필터 | 별도 가중치가 필요 없어 용량이 작고 빠르게 실행됩니다. 서버 부담이 적고 SAM이 실패했을 때 최소한의 결과를 보여주는 fallback으로 사용할 수 있습니다. | 진짜 생성형 AI가 아니라 눈·코·입·얼굴 윤곽 변화는 거의 만들지 못합니다. 주름, 잡티, 채도 감소 같은 필터 효과 중심이라 미래 얼굴처럼 보이기 어렵고 부자연스러울 수 있습니다. | 백업용. 최종 품질 기준 모델은 아닙니다. |

> SAM 선정 이유: SAM의 장점이 다른 모델보다 좋아 보여 단점은 깊게 고려하지 않고 우선 최종 후보로 선택했습니다. 특히 눈, 코, 입, 눈썹, 얼굴 윤곽 같은 인물 특징을 비교적 잘 유지하면서 target age를 넣어 10년 뒤부터 50년 뒤까지 결과를 만들 수 있다는 점이 프로젝트 목표와 가장 잘 맞았습니다.
### 5.1 SAM
...
- 특징:
  ...
  - Meta의 Segment Anything Model과는 다른 얼굴 나이 변환 모델
- 프로젝트 테스트 결과:
  - 로컬 실행 성공
