...
render_stage(
    ...
    "Streamlit 기반 로컬 웹앱",
    [
        ("Streamlit 기반 홈 화면 구성", "Python 모델과 결과 화면을 빠르게 연결하고 로컬에서 바로 확인하기 위해 사용했습니다."),
        ...
        ("SAM 미래 얼굴 생성 연결", "10년, 20년, 30년, 40년, 50년 뒤 target age별 결과를 만들기 위해 연결했습니다."),
    ...
    "웹앱 제작의 핵심은 카메라 촬영 → 현재 나이/성별 분석 → 미래 얼굴 시뮬레이션 → 결과 확인으로 이어지는 사용 흐름을 끊기지 않게 만드는 것이었습니다.",
)
...
render_stage(
    ...
    [
        ...
        (".gitignore 정리", "원본 데이터, 모델 가중치, 사용자 촬영 이미지, 대용량 로그가 저장소에 올라가지 않게 막았습니다."),
        ("README 작성", "프로젝트 목적, 기능, 모델 성능, 실행 방법, 필요한 모델 파일을 정리했습니다."),
        ("requirements.txt 작성", "로컬 또는 배포 환경에서 필요한 Python 패키지를 설치할 수 있게 했습니다."),
        ("필요한 코드/문서만 push", "GitHub 100MB 제한과 개인정보 문제를 피하기 위해 코드와 요약 자료 중심으로 업로드했습니다."),
    ],
    "배포를 고려하면 모든 파일을 올리는 것이 아니라, 재현 가능한 코드와 설명은 올리고 원본 데이터/가중치/개인 이미지는 분리해야 합니다.",
)
...
st.markdown(
    ...
    <div class="note-box">
      ...
      데이터셋 정리 → 전처리 검증 → 여러 모델 비교 학습 → 최종 모델 선택 → 미래 얼굴 후보 검토 → Streamlit 웹앱 제작 → GitHub 배포 준비까지 이어진 작업입니다.
    </div>
    """,
    unsafe_allow_html=True,
)
